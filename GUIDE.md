...
- **Recorded-session limits.** Rexec rejects WebSocket frames larger than 64 KiB (including framing bytes) and returns `503` above 128 concurrent recorded sessions or 16 per user. Kubectl sends stdin in 32 KiB chunks; custom clients must use similarly sized frames.
